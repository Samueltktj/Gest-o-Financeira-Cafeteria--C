...
void realizarVenda() {
  ...
  do {
    ...
    while (fscanf(cadastro, "%[^;];%d;%f\n", nome, &codigo, &preco) != EOF) {
      if (codigo == busca) {
        ...
        // Adiciona informações ao carrinho
        ...
        // Sair do loop após encontrar o produto
        break;
      }
    }
  ...
  for (int i = 0; i < indice; i++) {
    ...
    printf("Produto: %-25s| Quantidade: %-2d| Valor: R$%.2f\n",
...
void saidas() {
  ...
  Saida *saidas;
  saidas = (Saida *)malloc(20 * sizeof(Saida));
  ...
  if (saidas == NULL) {
    printf("Erro na alocação de memória saidas\n");
    return;
  }
